...
async function startPrompt() {
	...
	const checkBranchUpdate = execSync('git status -uno', {
		encoding: 'utf-8',
	});
		...
		const installDeps = await prompt([
			{
				...
				name: 'nodeModules',
				message:
					'Found new dependencies that require installation. Install them now?',
			},
		]);
...
function colorizeSpecificParts(text) {
	text = text.replace(/Starting.../g, chalk.hex('#021f42')('Starting...'));
	text = text.replace(/█/g, chalk.hex('#021f42')('█'));
